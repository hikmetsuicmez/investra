...
export async function POST(req: Request) {
	try {
		...
		const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/accounts/withdrawal`, {
			method: "POST",
			headers: {
		...
		return new Response(JSON.stringify({ message: "Bakiye hesaptan başarıyla çıkartıldı." }), { status: 201 });
	} catch (error) {
