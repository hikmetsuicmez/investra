...
export default function Dashboard() {
	...
	const renderValuationTable = () => {
		...
		if (valuations && valuations.length > 0) {
			const displayDate = new Date(valuations[0].valuationDate).toLocaleDateString("tr-TR");
				...
				<Card className="overflow-auto shadow-md">
					...
					<CardContent>
						<div className="flex items-center gap-4 mb-4">
							...
							{isSearchView && (
								<Button variant="outline" size="icon" onClick={handleClearSearch} title="Aramayı Temizle">
									<X className="h-4 w-4" />
								</Button>
							)}
						</div>
								...
								>
									...
									<div
										className={`text-right font-semibold ${
											v.unrealizedProfitLoss >= 0 ? "text-green-600" : "text-red-600"
										}`}
									>
									...
									<div
										className={`text-right font-semibold ${
											v.dailyChangePercentage >= 0 ? "text-green-600" : "text-red-600"
										}`}
									>
