...
public interface ClientApiDocs {
        ...
        @ApiResponse(responseCode = "500", description = "Sunucu hatası", content = @Content(schema = @Schema(implementation = Response.class)))
        ResponseEntity<Response<UpdateClientResponse>> updateClient(
                        @Parameter(description = "Güncellenecek müşteri ID'si", required = true) Long clientId,
                        @Parameter(description = "Müşteri güncelleme verileri (JSON)", required = true) Map<String, Object> payload);

        @Operation(summary = "Müşteri Bilgisi Getir", description = "Verilen kriterlere göre müşteriyi arar ve ilk bulunan müşterinin bilgilerini döner.", requestBody = @RequestBody(description = "Müşteri arama kriterleri", required = true, content = @Content(schema = @Schema(implementation = ClientSearchRequest.class))))
        ...
        @Operation(summary = "Müşteri Silme (Pasif Yapma)", description = "Verilen kriterlere göre müşteriyi arar ve bulunan ilk müşteriyi siler (pasif yapar).", requestBody = @RequestBody(description = "Silinecek müşteriyi belirlemek için arama kriterleri", required = true, content = @Content(schema = @Schema(implementation = ClientSearchRequest.class))))
        ...
        @Operation(summary = "Aktif Müşteri Listesi", description = "Sistemde aktif durumda olan tüm müşterileri listeler.")
        ...
        @Operation(summary = "Pasif Müşteri Listesi", description = "Sistemde pasif durumda olan tüm müşterileri listeler.")
